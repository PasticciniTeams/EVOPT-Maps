...
class ElectricVehicleNode(AstarNode): # Nodo per la ricerca A* con veicolo elettrico 
    def __init__(self, state, parent = None, action = None, g=0, h=0, veicle = None):
...
class ElectricVehicleAStar(AStar):
    def __init__(self, graph, heuristic, view = False, temperature = 20, veicle = None):
    ...
    def solve(self, problem):
        ...
        frontier.put(ElectricVehicleNode(problem.init, h=self.heuristic(problem.init, problem.goal), veicle = self.veicle))
        ...
        while not frontier.empty():
            n = frontier.get()
            if problem.isGoal(n.state, n.veicle.battery):
